...
The dashboard uses DAX to calculate KPI Match Rate.

### Match Rate

Business Purpose

Returns the percentage of reconciliation groups that are successfully matched.

```DAX
Match Rate =
DIVIDE(
    CALCULATE(
        COUNTROWS(vw_reconciliation_result),
        vw_reconciliation_result[reconciliation_status] = "MATCHED"
    ),
    COUNTROWS(vw_reconciliation_result)
)
```
